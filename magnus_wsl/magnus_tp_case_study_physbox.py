...
PHYSICAL-BOX VARIANT
--------------------
This file is ``magnus_tp_case_study.py`` with one change: NSFeas searches the
PHYSICAL box, T in [210, 300] and P in [5000, 11000], instead of the
normalised control box [-1, 1]^2. Everything else -- constraints, thresholds,
theta, NUMLIVE, NUMPROP, ELLMAG, ELLRED, the backend, the cache -- is
untouched.

Why it exists: to check whether normalising the search box affected the
comparison, since ``magnus_gtcd_case.py`` gives NSFeas its raw physical box
while this case study normalises. The prediction is that it makes NO
difference, because NSFeas's bound rule is affine-equivariant under per-axis
scaling:

  * the ellipsoid is the strict enclosing one, ``s = max ||L^-1 (x - mu)||``
    with ``L = chol(cov)``, and the Mahalanobis norm is invariant under any
    linear change of coordinates;
  * the box-versus-ellipsoid test in ``_sample_nest`` compares
    ``sum log(ub_i - lb_i)`` against ``sum log(shape_ii)``, and rescaling axis
    i by c_i shifts BOTH sides by ``sum log(c_i)``, so the margin is unchanged.

Checked numerically on the recorded final live clouds of this case study and
of GTCD: identical decision margin to six decimals, and zero membership
disagreements over 200 000 random points in each.

MEASURED: the two searches are not merely similar, they are the same run.

Full HYSYS, N_L = 500, against the recorded normalised run
(``magnus_tp_output/magnus_tp_bridge_NL500.npz``):

                            normalised     physical
    accepted (dead)                657          657
    rejected (discarded)           455          455
    candidates                    1112         1112
    evaluations                   1612         1612
    efficiency                  0.5908       0.5908

And not only the counts. Pool the live, dead and rejected arrays, sort them,
and the largest coordinate difference across all 1612 points is 2.3e-11, with
none exceeding 1e-6. The physical-box search visited the same points and made
the same accept/reject decision at every one of them.

Sorting first is necessary. The arrays are stored in merit order, and the
criterion values differ between the two runs by up to 1e-3 -- HYSYS's own
repeatability on this flowsheet, measured by solving one point twice in a row
and getting -0.170644 and -0.166928. That reorders near-ties, so an
element-wise comparison of the live and dead arrays shows differences of
thousands of kPa. Those are an artefact of the ordering, nothing else; the
rejected array, which is not merit-sorted, agrees to 2.3e-11 without sorting.

The point worth keeping: that 1e-3 of solver noise flipped no accept/reject
decision anywhere in the run.

The surrogate backend agrees too, at three live-point counts, in about a
second each:

    N_L    box           dead    disc    cand     eff
    100    normalised     137      79     216   0.634
    100    physical       137      79     216   0.634
    300    normalised     398     258     656   0.607
    300    physical       398     258     656   0.607
    500    normalised     677     475    1152   0.588
    500    physical       677     475    1152   0.588

WHY IT COMES OUT IDENTICAL: ``_sample_nest`` draws its randomness in the unit
hyperball -- ``normalise(randn(nu)) * randu()^(1/nu)`` -- and only then maps
it through ``_nestCentre + _nestShape * vSAM``. The random numbers therefore
do not depend on the search coordinates at all, and the affine map carries the
same ball point to the same physical point either way.

Not bit-for-bit, though, and that is the useful part: the 2.3e-11 is the
rounding you get from performing the same affine map in a different order. A
change that had failed to take effect would give exactly zero. So the
arithmetic path really is different, and the agreement is a result rather than
a tautology.

PROVENANCE of the HYSYS run: it was assembled over three sessions, because
HYSYS exhausted memory twice on this 8 GB machine and had to be restarted. Its
npz therefore reports ``warm_cache_hits = 1096`` against 516 fresh solves, and
its wall clock is not a measurement of anything. Every one of the 1612 solves
is this script's own, made while searching the physical box -- the cache is
written by this run and read back by it. Only the timings are meaningless.

Output goes to ``magnus_tp_physbox_*`` so it cannot overwrite the normalised
run's files.

COST: a full HYSYS run is 1612 solves at 10-12 s each, so five to six hours on
the bridge, and it needs enough free memory to survive that long. Use
``--backend surrogate`` first to check the wiring in about a second.
...
def to_physical(u):
    """Identity: in this variant the control point IS (T [C], P [kPa]).

    The normalised script maps [-1, 1]^2 onto the physical box here. This one
    hands NSFeas the physical box directly (see ``set_control`` below), so the
    control point arrives already in physical units and there is nothing to
    map. Kept as a function, with the same name and call site, so the rest of
    the file is byte-for-byte the normalised version.
    """
    return [float(value) for value in u]
...
def run(args):
    ...
    # THE change against magnus_tp_case_study.py: the physical box goes in as
    # the control box, rather than [-1, 1]^2 with to_physical() in between.
    NS.set_control(d,
                   [float(lo) for lo, _ in DESIGN_BOUNDS],
                   [float(hi) for _, hi in DESIGN_BOUNDS])
...
def report_and_save(NS, args, limits, cache, timing, _keep_alive):
    ...
    def physical(points):
        # Identity here: NSFeas searched the physical box, so the points it
        # returns are already (T, P). The normalised script rescales from
        # [-1, 1]^2 at this point. Everything downstream -- the reported
        # bounding box, the edge test, the npz and csv -- reads physical
        # values, so it is unchanged.
    ...
    # "physbox" in the default tag so this variant can never overwrite the
    # normalised run's magnus_tp_bridge_NL500.{npz,csv}.
    tag = args.tag or f"physbox_{args.backend}_NL{args.numlive}"
...
def main(argv=None):
    ...
    # "physbox" in the default so this variant's log, npz and csv can never
    # overwrite the normalised run's magnus_tp_bridge_NL500.*
    args.tag = args.tag or f"physbox_{args.backend}_NL{args.numlive}"
